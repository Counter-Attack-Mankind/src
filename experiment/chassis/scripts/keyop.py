...
import roslib
import rospy
from sandbox_msgs.msg import ChassisCommand
...
class AckermannDriveKeyop:
    ...
    def __init__(self, args):
        ...
        rospy.Timer(rospy.Rate(10).sleep_dur, self.pub_callback, oneshot=False)
        self.print_state()
        self.key_loop()

    def pub_command(self, target, throttle, steering):
        chassis_command = ChassisCommand()
        chassis_command.target = target
        chassis_command.throttle = throttle
        chassis_command.steering = steering
        self.motors_pub.publish(chassis_command)

    def pub_callback(self, event):
        self.pub_command(self.target, self.speed, self.steering_angle)
    ...
    def print_state(self):
        ...
        rospy.loginfo('\x1b[1M\rUse number keys to select target')
        rospy.loginfo('\x1b[1M\rPress <ctrl-c> or <q> to exit')
        rospy.loginfo('\x1b[1M\r*********************************************')
        rospy.loginfo('\x1b[1M\r'
                      '\033[34;1mTarget: \033[32;1m%d, '
                      '\033[34;1mSpeed: \033[32;1m%0.2f, '
                      '\033[34;1mSteer Angle: \033[32;1m%0.2f\033[0m',
                      self.target, self.speed, self.steering_angle)
    ...
    def finalize(self):
        ...
        self.settings = termios.tcgetattr(sys.stdin)
        self.pub_command(self.target, 0, 0)
        sys.exit()
