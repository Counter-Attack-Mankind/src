...
void spline::set_points(const std::vector<double>& x,
                        const std::vector<double>& y, bool cubic_spline, bool equally_distrubuted)
{
  assert(x.size()==y.size());
  assert(x.size()>2);
  m_x=x;
  m_y=y;
  m_equally_distrubuted=equally_distrubuted;
  if(equally_distrubuted) {
    m_resolution = x[1] - x[0];
  }
  int   n=x.size();
...
double spline::operator() (double x) const
{
  ...
  int idx = 0;
  if(m_equally_distrubuted) {
    idx = std::min(std::max((int)floor(x / m_resolution)-1, 0), (int)n - 1);
  } else {
