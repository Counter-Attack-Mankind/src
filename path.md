...
2026-07-27：将 A1 服务权从“首辆进入 TO_A1 即提前锁定”改为请求线准入。车辆只有在沿当前固定路径的 `remainingS` 不大于 `a1_request_distance`（默认 1.50m）后才参与正式授予，请求线外保持无 owner 并继续普通交通仲裁；同周期多车请求时选择 `remainingS` 更小者、以 id 终裁。正式授予后仍锁定到装货并清出 A1。请求线运行时保证位于 1.10m 候车停止线之外，并覆盖最大速度制动距离、一个决策周期反应距离和额外余量。

2026-07-27：扩展 `multi_vehicle_coordination.log` 的等待环现场诊断。环持续 1s 后写入 `[coord_diag]`，包含环成员、A1 owner、车辆任务阶段与路径版本、普通冲突预约、所有剩余 OBB 冲突块的坐标/方向/停止线/当前真实占用、按 NOMINAL 预测的进入退出时间，以及整片外包区 committed 是否在“两车并未同时真实占区”时制造双刹；同一环每 5s 更新一次。

2026-07-28：A1-cycle 改为“提前定任务、分离执行航段”。车辆离开当前 B 库位前即随机选择并软预约下一目标 B，同时缓存独立的 `A1->B` 为 `pending_dropoff_track`；活动 `track` 仍只有 `B->A1`，普通两车仲裁不会把两段路径拼接成一个大冲突资源。A1 装货完成后不再临时重新选靶，只将已预约航段原子切换为活动航段。A1 服务车在进场/装货期间会用待执行离场路径与候车当前 `B->A1` 路径计算精确 OBB 冲突；候车停止线取固定 A1 安全线和未来离场最早冲突入口中更靠前者，日志原因为 `wait_a1_reserved_exit_Vx`。除固定 1.50m 请求线外，候车接近该精确门线的制动时机也会触发服务顺序裁决，避免“服务权尚未产生、候车先越线”；它只提前确定 owner，其他车仍可运行到各自门线。若初始化等异常状态下车辆已经越线，输出 `[A1 gate late]`。

2026-07-28：重做同向跟车关系。`resolveFollowing` 不再从 `(a,b)`、`(b,a)` 两个方向独立判断，而是每个无序车对只处理一次，按当前同向共享块上的归一化路径进度生成唯一 `leader/follower`；同向关系一旦成立即在 RViz 持续显示浅蓝区域，不再与是否已经减速绑定。leader 不受自己的 follower 和前向净空护栏反向制动，follower 仍按间距使用 `NOMINAL/YIELD/CREEP/STOP`。同一车对若另有交叉/对向块，pairwise 只移除已由跟车接管的同向块，其他块继续仲裁。A1 owner 与 TO_A1 候车禁止注册普通跟车关系，避免跟车层绕过 A1 事务优先级；普通停止线增加 0.02m 静态闭锁带，防止车辆静止后被 `action_hold` 蹭过门线。死锁恢复不再把空载 TO_A1 车辆直接重规划成 TO_B。

2026-07-28：修正换向路径被误判为整段同向跟车的问题。两车冲突几何现在会在任一方 `FORWARD/REVERSE` 切换及实际运动朝向类别变化处拆分，冲突块不再跨越换向点合并。普通跟车仅允许档位阶段一致的 `F/F` 或 `R/R` 子区，并要求两车车身已进入该子区、当前运动朝向仍一致，且各自距离换向点至少 0.15m；`F/R`、`R/F` 以及换向保护带内统一交回普通互斥仲裁。pairwise 只移除当前真正由跟车接管的子区，混合档位和未来子区继续受保护。协调日志增加车辆当前 `gear=F/R` 和冲突区 `phase=F/F、R/R、F/R、R/F`。

2026-07-28：扩大 A1 服务区并修复装货换轨时的门线断点。候车门线现在取“1/2 走廊左右竖直连接车道中部的稳定竖直姿态点、A1 离场路径对该候车路径的最早精确 OBB 冲突停止线、原固定距离停止线”三者中最靠前者；车辆接近竖直门线即参与 A1 服务权申请。A1 owner 完成装货后，门控几何从 `pending_dropoff_track` 无缝切换到活动 `track`，不会因 pending 被清空而释放候车。日志增加 `source=vertical/pending/active`。RViz 新增紫色 U 形 `a1_service_zone`，覆盖第一走廊及两侧竖直连接车道的上半段，并以高亮紫线显示左右候车线。
