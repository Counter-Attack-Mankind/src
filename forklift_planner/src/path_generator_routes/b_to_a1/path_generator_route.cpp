...
RoughPath PathGenerator::generateRouteBToA1(const Slot& src, const Slot& tgt,
    ...
    const bool target_is_depot_a1 = (tgt.id == 101);
    const bool target_is_endpoint = tgt.id < 0 || target_is_depot_a1;
    ...
    if (debug_row1_target) {
        ROS_WARN("[planner][row1-debug] route=%s debug_rev=row1_aux_lane_to_a1_v2 "
    ...
    auto transition_x = [&](int from_corr, int to_corr, double current_x, double future_x) {
        ...
        if ((from_corr == 3 && to_corr == 4) || (from_corr == 4 && to_corr == 3)) {
            ...
            if (!going_down && target_is_depot_a1 && src.row_id == 7) {
                return (src.col <= 4) ? row3_up_x : row3_down_x;
            }
            if (going_down) {
    ...
    auto current_transition_lane_y = [&](int current_corr, int next_corr) {
        ...
        if (target_is_depot_a1 && src.row_id == 7 &&
            current_corr == 4 && next_corr == 3) {
            return corridor_lane_y(mp_, current_corr, HDir::LEFT);
        }
        if (target_is_depot_a1 &&
            (src.row_id == 6 || src.row_id == 7) &&
            current_corr == 3 && next_corr == 2) {
            return corridor_lane_y(mp_, current_corr, HDir::LEFT);
        }
    ...
    const bool row1_upper_source_to_a1 =
        target_is_depot_a1 && src.row_id == 1;
    const bool row1_lower_right_source_to_a1 =
        target_is_depot_a1 && src.row_id == 2 && src.col >= 3;
    const bool row2_upper_left_to_a1 =
        target_is_depot_a1 && src.row_id == 3 && src.col <= 4;
    const bool row2_upper_right_to_a1 =
        target_is_depot_a1 && src.row_id == 3 && src.col >= 5;
    const bool row2_upper_left_edge_to_a1 =
    ...
    const bool row4_bottom_left_to_a1 =
        target_is_depot_a1 && src.row_id == 7 && src.col <= 4;
    const bool row4_bottom_right_to_a1 =
        target_is_depot_a1 && src.row_id == 7 && src.col >= 5;
    auto row1_upper_aux_lane_y = [&]() {
        const double upper_lane_y = corridor_lane_y(mp_, 1, HDir::LEFT);
        const double lower_lane_y = corridor_lane_y(mp_, 1, HDir::RIGHT);
        const double source_rear_y =
            src.dock_y() -
            mp_.rear_axle_to_center * std::sin(src.dock_theta);
        const double min_source_gap =
            std::max(final_min_req_y + 0.02, 0.36);
        const double max_safe_y =
            std::max(lower_lane_y + sample_ds,
                     source_rear_y + min_source_gap);
        double aux_y = std::min(upper_lane_y - 0.10, max_safe_y);
        aux_y = std::max(lower_lane_y + 0.06,
                         std::min(upper_lane_y - 0.04, aux_y));
        return aux_y;
    };
    ...
    if (src.id >= 0) {
        ...
        bool start_to_right = same_corridor
        ...
        const bool row1_lower_to_a1 =
            target_is_depot_a1 && src.row_id == 2;
        double row0_outer_exit_x = start_ref_x;
        if (row1_lower_right_source_to_a1) {
            start_ref_x = row1_right_down_x;
            start_to_right = true;
        }
        if (row0_outer_to_a1) {
            const double depot_side_run =
                std::max(0.48, final_min_req_x + 0.24);
            row0_outer_exit_x = row0_left_outer_to_a1
                ? std::min(mp_.field_width - 0.04, spine_x_ + depot_side_run)
                : std::max(0.04, spine_x_ - depot_side_run);
            start_ref_x = row0_outer_exit_x;
            start_to_right = row0_right_outer_to_a1;
        }
        if (!same_corridor &&
            ...
            } else if (row1_lower_right_source_to_a1) {
                first_transition_x = row1_right_down_x;
            } else {
                first_transition_x =
        ...
        if (row1_upper_to_a1) {
            ...
            if (debug_row1_target) {
                const double upper_lane_y =
                    corridor_lane_y(mp_, src_corr, HDir::LEFT);
                const double lower_lane_y =
                    corridor_lane_y(mp_, src_corr, HDir::RIGHT);
                const double source_rear_y =
                    src.dock_y() -
                    mp_.rear_axle_to_center * std::sin(src.dock_theta);
                ROS_WARN("[planner][row1-debug] row1-upper first lane to A1 "
                         "src=%d lower=%.3f upper=%.3f start_lane_y=%.3f "
                         "source_rear_y=%.3f",
                         src.id, lower_lane_y, upper_lane_y, start_lane_y,
                         source_rear_y);
            }
            ...
            start_lane_y = corridor_lane_y(mp_, src_corr, HDir::RIGHT);
        } else if (row4_bottom_left_to_a1 || row4_bottom_right_to_a1) {
            start_lane_y = corridor_lane_y(mp_, src_corr, HDir::LEFT);
        }
        ...
        const Pt rear_axle_start{
            src.dock_x() - mp_.rear_axle_to_center * std::cos(src.dock_theta),
            src.dock_y() - mp_.rear_axle_to_center * std::sin(src.dock_theta)};
        const Pt reverse_origin =
            row1_upper_to_a1 ? rear_axle_start : Pt{src.dock_x(), src.dock_y()};
        const double vertical_space =
            std::abs((start_lane_y - reverse_origin.y) * reverse_start_dir.y);
        auto fit_initial_reverse = [&](const Pt& reverse_end_dir,
            ...
            const double horizontal_space =
                (reverse_end_dir.x < 0.0)
                    ? (reverse_origin.x - 0.04)
                    : (mp_.field_width - reverse_origin.x - 0.04);
            ...
            if (row1_upper_to_a1) {
                const double compact_tangent =
                    std::min(0.72 * std::max(0.0, vertical_space),
                             0.72 * std::max(0.0, horizontal_space));
            ...
            if (candidate.pts.empty() && !row1_upper_to_a1) {
                const double compact_tangent =
            ...
            const double straight_len =
                (start_lane_y - reverse_origin.y - curve_delta.y) /
                reverse_start_dir.y;
            const Pt curve_start{
                reverse_origin.x + reverse_start_dir.x * straight_len,
                reverse_origin.y + reverse_start_dir.y * straight_len};
        ...
        if (same_corridor || src_corr != tgt_corr) {
            ...
            if (row1_upper_to_a1) {
                (void)reserve_x;
                direct_reverse_end_x = src.pre_dock_x;
            } else {
            ...
            if (row2_upper_left_edge_to_a1) {
                ...
                direct_reverse_end_x =
                    std::min(mp_.field_width - 0.04,
                             row3_up_x + min_exit_run);
            } else if (row4_bottom_right_to_a1) {
                const double min_exit_run =
                    std::max(0.36, final_min_req_x + 0.06);
                direct_reverse_end_x =
                    std::max(0.04, row3_down_x - min_exit_run);
            }
            if (row0_outer_to_a1) {
        ...
        if (debug_row1_target) {
            ROS_WARN("[planner][row1-debug] initial_reverse tgt=%d same_corr=%d "
                     ...
                     "actual_end=(%.3f,%.3f) reverse_origin=(%.3f,%.3f) "
                     "curve_start=(%.3f,%.3f) curve_pts=%zu direct_ok=%d",
                     ...
                     reverse_origin.x, reverse_origin.y,
    ...
    while (current_corr != tgt_corr) {
        ...
        if (current_corr == 3 && next_corr == 2) {
            ...
            if (row3_lower_left_to_a1 || row3_lower_right_to_a1 ||
                row4_bottom_left_to_a1 || row4_bottom_right_to_a1) {
                const double lower_lane_y =
                    corridor_lane_y(mp_, current_corr, HDir::RIGHT);
                const double upper_lane_y =
                    corridor_lane_y(mp_, current_corr, HDir::LEFT);
                const double next_lane_y =
                    corridor_lane_y(mp_, next_corr, HDir::RIGHT);
                connector_x = spine_x_;
                push_point(polyline, {current_x, lower_lane_y});
                push_point(polyline, {current_x, upper_lane_y});
                push_point(polyline, {connector_x, upper_lane_y});
                push_point(polyline, {connector_x, next_lane_y});
                if (debug_row1_target) {
                    ROS_WARN("[planner][row1-debug] row3lower 3to2 staged shift "
                             "src=%d side=%s lower=(%.3f,%.3f) "
                             "upper=(%.3f,%.3f) bridge=(%.3f,%.3f) "
                             "next=(%.3f,%.3f)",
                             src.id,
                             (row3_lower_left_to_a1 || row4_bottom_left_to_a1)
                                 ? "left" : "right",
                             current_x, lower_lane_y,
                             current_x, upper_lane_y,
                             connector_x, upper_lane_y,
                             connector_x, next_lane_y);
                }
    ...
    if (target_is_depot_a1 && src.row_id == 1) {
        goal_lane_y = current_y;
    }
    ...
    auto prepend_initial_reverse = [&](RoughPath& path) {
        ...
        if (debug_row1_target) {
            ROS_WARN("[planner][row1-debug] prepend enter src=%d row1_upper=%d "
                     "raw_pts=%zu a_rev=(%.3f,%.3f) b=(%.3f,%.3f) "
                     "path_front=(%.3f,%.3f %.1fdeg type=%d)",
                     src.id, row1_upper_source_to_a1 ? 1 : 0,
                     pts.size(), a_rev.x, a_rev.y, b.x, b.y,
                     path.front().x, path.front().y,
                     path.front().theta * 180.0 / kPi,
                     static_cast<int>(path.front().type));
        }

        RoughPath prefix;
        prefix.reserve(pts.size() + path.size());
        auto append_chord_based_reverse_prefix = [&]() {
            for (size_t k = 0; k < pts.size(); ++k) {
                Pt nxt;
                if (k + 1 < pts.size()) {
                    nxt = pts[k + 1];
                } else {
                    nxt = Pt{path.front().x, path.front().y};
                }
                double motion;
                if (dist(pts[k], nxt) > 1e-9) {
                    motion = std::atan2(nxt.y - pts[k].y, nxt.x - pts[k].x);
                } else if (k > 0) {
                    const Pt prev = pts[k] - pts[k - 1];
                    motion = std::atan2(prev.y, prev.x);
                } else {
                    motion = norm_angle(src.dock_theta + kPi);
                }
                prefix.push_back({pts[k].x, pts[k].y,
                                  norm_angle(motion + kPi), type});
            }
            if (!prefix.empty()) {
                prefix.back().x = b.x;
                prefix.back().y = b.y;
            ...
            if (debug_row1_target) {
                ROS_WARN("[planner][row1-debug] chord reverse prefix src=%d "
                         "pts=%zu first=(%.3f,%.3f %.1fdeg) "
                         "last=(%.3f,%.3f %.1fdeg)",
                         src.id, prefix.size(),
                         prefix.empty() ? 0.0 : prefix.front().x,
                         prefix.empty() ? 0.0 : prefix.front().y,
                         prefix.empty() ? 0.0 :
                             prefix.front().theta * 180.0 / kPi,
                         prefix.empty() ? 0.0 : prefix.back().x,
                         prefix.empty() ? 0.0 : prefix.back().y,
                         prefix.empty() ? 0.0 :
                             prefix.back().theta * 180.0 / kPi);
                for (size_t dbg_i = 0; dbg_i < prefix.size(); ++dbg_i) {
                    if (dbg_i >= 4 &&
                        std::abs(prefix[dbg_i].y - src.dock_y()) > 0.025 &&
                        std::abs(prefix[dbg_i].y - 3.791) > 0.025) {
                        continue;
                    }
                    ROS_WARN("[planner][row1-debug] prefix[%zu]=(%.3f,%.3f "
                             "%.1fdeg type=%d)",
                             dbg_i, prefix[dbg_i].x, prefix[dbg_i].y,
                             prefix[dbg_i].theta * 180.0 / kPi,
                             static_cast<int>(prefix[dbg_i].type));
                }
            }
            prefix.insert(prefix.end(), path.begin(), path.end());
            path.swap(prefix);
        };
        if (row1_upper_source_to_a1) {
            append_chord_based_reverse_prefix();
            return;
        }
        if (!initial_reverse_curve.pts.empty()) {
    ...
    auto build_exact_a1_lane_path = [&]() {
        RoughPath path;
        const double first_lane_y = corridor_lane_y(mp_, 1, HDir::LEFT);
        const double second_lane_y = corridor_lane_y(mp_, 1, HDir::RIGHT);
        const double top_lane_y = row1_upper_source_to_a1
            ? second_lane_y
            : first_lane_y;
        const double map_mt_port = 0.30;
        const double rear_stop_y =
            tgt.dock_y() -
            mp_.rear_axle_to_center * std::sin(tgt.dock_theta);
        const double body_margin = 0.01;
        const double max_safe_rear_y =
            mp_.field_height - mp_.body_front_ext() - body_margin;
        const double map_safe_out =
            std::max(rear_stop_y - top_lane_y,
                     max_safe_rear_y - top_lane_y);
        const double available_out =
            std::max(sample_ds, std::min(map_mt_port, map_safe_out));
        const double mt_port = available_out;
        const Pt lane_start = initial_reverse_end;
        const double row1_connector_x = row1_upper_source_to_a1
            ? ((src.pre_dock_x < spine_x_) ? row1_right_down_x
                                           : row1_left_up_x)
            : lane_start.x;
        const int din_sign = (row1_connector_x <= spine_x_) ? 1 : -1;
        const double in_heading = (din_sign > 0) ? 0.0 : kPi;
        const double signed_turn =
            (din_sign > 0) ? (kPi * 0.5) : (-kPi * 0.5);
        TurnCurve curve = fit_clothoid_turn(signed_turn, max_curvature,
                                            steer_ramp_len, mt_port,
                                            sample_ds);
        if (curve.pts.empty()) {
            curve = fit_clothoid_turn(signed_turn, max_curvature,
                                      sample_ds, mt_port, sample_ds);
        }
        if (curve.pts.empty()) {
            curve = build_g2_spiral_turn(signed_turn, available_out,
                                         sample_ds);
            if (!curve.pts.empty() && debug_row1_target) {
                ROS_WARN("[planner][row1-debug] exact A1 lane using "
                         "terminal-constrained G2 fallback src=%d din=%d "
                         "t_in=%.3f t_out=%.3f mt_port=%.3f "
                         "available_out=%.3f rear_stop_y=%.3f "
                         "max_safe_rear_y=%.3f",
                         src.id, din_sign, curve.t_in, curve.t_out,
                         mt_port, available_out, rear_stop_y,
                         max_safe_rear_y);
            }
        }
        if (curve.pts.empty()) {
            if (debug_row1_target) {
                ROS_WARN("[planner][row1-debug] exact A1 lane failed src=%d "
                         "din=%d lane_start=(%.3f,%.3f) top_lane_y=%.3f "
                         "rear_stop_y=%.3f max_safe_rear_y=%.3f "
                         "mt_port=%.3f",
                         src.id, din_sign, lane_start.x, lane_start.y,
                         top_lane_y, rear_stop_y, max_safe_rear_y,
                         mt_port);
            }
            return path;
        }
        const Pt curve_start{spine_x_ - din_sign * curve.t_in, top_lane_y};
        auto push_wp = [&](const Pt& p, double theta, WpType type) {
            if (!path.empty() &&
                std::hypot(path.back().x - p.x, path.back().y - p.y) < 1e-5) {
                path.back().theta = norm_angle(theta);
                path.back().type = type;
                return;
            }
            path.push_back({p.x, p.y, norm_angle(theta), type});
        };
        auto append_line = [&](const Pt& a, const Pt& b, double theta,
            ...
            for (int k = 0; k <= steps; ++k) {
                ...
                push_wp({a.x + (b.x - a.x) * u,
                         a.y + (b.y - a.y) * u},
                        theta, type);
            }
        };
        auto append_road_arc = [&](const Pt& corner, const Pt& u_in,
                                   const Pt& u_out, double radius,
                                   WpType type = WpType::FORWARD) {
            const double bend = cross(u_in, u_out);
            if (std::abs(bend) < 1e-6 || radius <= sample_ds) return;
            const Pt t1 = corner - u_in * radius;
            const Pt t2 = corner + u_out * radius;
            const Pt normal =
                (bend > 0.0) ? left_normal(u_in) : right_normal(u_in);
            const Pt center = t1 + normal * radius;
            ...
            const int steps = std::max(
                2,
                static_cast<int>(std::ceil(std::abs(a1 - a0) * radius /
                                           sample_ds)));
            for (int k = 1; k <= steps; ++k) {
                const double u = static_cast<double>(k) /
                                 static_cast<double>(steps);
                const double ang = a0 + (a1 - a0) * u;
                const Pt p{center.x + radius * std::cos(ang),
        ...
        if (row1_upper_source_to_a1) {
            const double connector_x = row1_connector_x;
            const double connector_mid_y = 0.5 * (mp_.y6() + mp_.y7());
            const Pt s0{
                src.dock_x() -
                    mp_.rear_axle_to_center * std::cos(src.dock_theta),
                src.dock_y() -
                    mp_.rear_axle_to_center * std::sin(src.dock_theta)};
            const Pt s1{src.pre_dock_x, first_lane_y};
            const Pt s2{connector_x, first_lane_y};
            const Pt s3{connector_x, connector_mid_y};
            const Pt s4{connector_x, second_lane_y};
            const Pt s5{spine_x_, second_lane_y};
            const Pt s6{
                tgt.dock_x() -
                    mp_.rear_axle_to_center * std::cos(tgt.dock_theta),
                tgt.dock_y() -
                    mp_.rear_axle_to_center * std::sin(tgt.dock_theta)};
            path.clear();
            if (info != nullptr) {
                info->debug_layers.clear();
                record_debug_layer(info, DebugPathLayerType::SKELETON,
                                   "route_skeleton",
                                   std::vector<Pt>{s0, s1, s2, s3, s4, s5, s6});
            }

            auto append_motion_line = [&](const Pt& a, const Pt& b,
                                          WpType type,
                                          bool include_start = true) {
                const double len = dist(a, b);
                if (len <= 1e-6) return;
                const double motion = std::atan2(b.y - a.y, b.x - a.x);
                const double body = (type == WpType::REVERSE)
                    ? norm_angle(motion + kPi)
                    : motion;
                const int steps =
                    std::max(1, static_cast<int>(std::ceil(len / sample_ds)));
                const int first_step = include_start ? 0 : 1;
                for (int k = first_step; k <= steps; ++k) {
                    const double u = static_cast<double>(k) /
                                     static_cast<double>(steps);
                    push_wp({a.x + (b.x - a.x) * u,
                             a.y + (b.y - a.y) * u},
            ...
            auto append_motion_arc = [&](const Pt& corner, const Pt& u_in,
                                         const Pt& u_out, double radius,
                                         WpType type) {
                append_road_arc(corner, u_in, u_out, radius, type);
            };

            const double r_reverse = std::max(
                sample_ds,
                std::min({std::min(0.30, 0.95 / std::max(max_curvature, kEps)),
                          0.45 * dist(s0, s1),
                          0.45 * dist(s1, s2),
                          0.45 * dist(s2, s3)}));
            const Pt r_u0 = normalize(s1 - s0);
            const Pt r_u1 = normalize(s2 - s1);
            const Pt r_u2 = normalize(s3 - s2);
            append_motion_line(s0, s1 - r_u0 * r_reverse, WpType::REVERSE);
            append_motion_arc(s1, r_u0, r_u1, r_reverse, WpType::REVERSE);
            append_motion_line(s1 + r_u1 * r_reverse,
                               s2 - r_u1 * r_reverse, WpType::REVERSE);
            append_motion_arc(s2, r_u1, r_u2, r_reverse, WpType::REVERSE);
            append_motion_line(s2 + r_u2 * r_reverse, s3, WpType::REVERSE);

            const double r_forward = std::max(
                sample_ds,
                std::min({std::min(0.30, 0.95 / std::max(max_curvature, kEps)),
                          0.45 * dist(s3, s4),
                          0.45 * dist(s4, s5),
                          0.45 * dist(s5, s6)}));
            const Pt f_u0 = normalize(s4 - s3);
            const Pt f_u1 = normalize(s5 - s4);
            const Pt f_u2 = normalize(s6 - s5);
            append_motion_line(s3, s4 - f_u0 * r_forward,
                               WpType::FORWARD, false);
            append_motion_arc(s4, f_u0, f_u1, r_forward, WpType::FORWARD);
            append_motion_line(s4 + f_u1 * r_forward,
                               s5 - f_u1 * r_forward, WpType::FORWARD);
            append_motion_arc(s5, f_u1, f_u2, r_forward, WpType::FORWARD);
            append_motion_line(s5 + f_u2 * r_forward, s6, WpType::FORWARD);
            if (debug_row1_target) {
                ROS_WARN("[planner][row1-debug] row1-upper full road to A1 "
                         "S0=(%.3f,%.3f) S1=(%.3f,%.3f) "
                         "S2=(%.3f,%.3f) S3=(%.3f,%.3f) "
                         "S4=(%.3f,%.3f) S5=(%.3f,%.3f) "
                         "S6=(%.3f,%.3f) r=(%.3f,%.3f) pts=%zu",
                         s0.x, s0.y, s1.x, s1.y,
                         s2.x, s2.y, s3.x, s3.y,
                         s4.x, s4.y, s5.x, s5.y,
                         s6.x, s6.y, r_reverse, r_forward,
                         path.size());
            }
        ...
        append_line(lane_start, curve_start, in_heading);
        const double ch = std::cos(in_heading);
        const double sh = std::sin(in_heading);
        for (size_t j = 1; j < curve.pts.size(); ++j) {
            const Pt p{curve_start.x + ch * curve.pts[j].x -
                           sh * curve.pts[j].y,
                       top_lane_y + sh * curve.pts[j].x +
                           ch * curve.pts[j].y};
            push_wp(p, in_heading + curve.headings[j], WpType::FORWARD);
        }
        const Pt curve_end{curve_start.x + ch * curve.pts.back().x -
                               sh * curve.pts.back().y,
                           top_lane_y + sh * curve.pts.back().x +
                               ch * curve.pts.back().y};
        if (curve_end.y < rear_stop_y - 1e-5) {
            append_line(curve_end, {spine_x_, rear_stop_y}, kPi * 0.5);
        } else if (curve_end.y > rear_stop_y + 1e-5) {
            append_line(curve_end, {spine_x_, rear_stop_y}, kPi * 0.5,
                        WpType::REVERSE);
        }

        if (debug_row1_target) {
            ROS_WARN("[planner][row1-debug] exact A1 lane src=%d "
                     "din=%d lane_start=(%.3f,%.3f) "
                     "curve_start=(%.3f,%.3f) curve_end=(%.3f,%.3f) "
                     "rear_stop_y=%.3f max_safe_rear_y=%.3f mt_port=%.3f "
                     "available_out=%.3f finish_mode=%s pts=%zu",
                     src.id, din_sign, lane_start.x, lane_start.y,
                     curve_start.x, curve_start.y,
                     curve_end.x, curve_end.y,
                     rear_stop_y, max_safe_rear_y, mt_port, available_out,
                     (curve_end.y > rear_stop_y + 1e-5) ? "reverse_trim" :
                     ((curve_end.y < rear_stop_y - 1e-5) ? "forward_extend" :
                      "on_stop"),
                     path.size());
        }
    ...
    if (target_is_depot_a1 && src.row_id == 1 && !terminal_reverse) {
        RoughPath exact_lane_path = build_exact_a1_lane_path();
        if (!exact_lane_path.empty()) {
            return exact_lane_path;
        }
    }
    ...
    for (size_t j = 1; j + 2 < n; ++j) {
        ...
        if (target_is_depot_a1 && j + 2 == n - 1) {
            if (debug_row1_target) {
                ROS_WARN("[planner][row1-debug] lane_shift skipped tgt=%d j=%zu "
                         "terminal A1 connector keeps full vertical segment",
                         tgt.id, j);
            }
            continue;
        }
        ...
        const bool row12_right_inner_corr2_to_corr1_shift =
            ...
            std::abs(simplified[j].y -
                     corridor_lane_y(mp_, 2, HDir::LEFT)) < 0.03 &&
            std::abs(simplified[j + 1].y -
                     corridor_lane_y(mp_, 2, HDir::LEFT)) < 0.03 &&
            std::abs(simplified[j + 1].x - row1_right_down_x) < 0.03;
        if (row12_right_inner_corr2_to_corr1_shift) {
            if (debug_row1_target) {
                ROS_WARN("[planner][row1-debug] lane_shift skipped tgt=%d j=%zu "
                         "row12 right-inner keeps clothoid/local-arc middle connector "
                         "lateral=%.3f",
                         tgt.id, j, lateral);
            }
            continue;
        ...
        const bool row3lower_corr3_to_corr2_shift =
            target_is_depot_a1 &&
            (row3_lower_left_to_a1 || row3_lower_right_to_a1 ||
             row4_bottom_left_to_a1 || row4_bottom_right_to_a1) &&
            std::abs(simplified[j].y -
                     corridor_lane_y(mp_, 3, HDir::LEFT)) < 0.03 &&
            std::abs(simplified[j + 1].y -
                     corridor_lane_y(mp_, 3, HDir::LEFT)) < 0.03 &&
            std::abs(simplified[j + 1].x - spine_x_) < 0.03;
        if (row3lower_corr3_to_corr2_shift) {
            min_total = std::min(avail_in + avail_out,
                                 std::max(min_total, 1.20));
        }
        ...
        if (debug_row1_target) {
            ROS_WARN("[planner][row1-debug] lane_shift accepted tgt=%d j=%zu "
                     ...
                     tgt.id, j, terminal_lane_shift ? 1 : 0,
                     lateral, lead_in, lead_out,
                     row3lower_corr3_to_corr2_shift ? 1 : 0);
        }
    ...
    for (size_t j = 1; j + 1 < n; ++j) {
        ...
        if (j + 1 == n - 1) {
            const double terminal_out_ratio = target_is_depot_a1 ? 1.0 : 0.99;
            turn_limits[j] = std::min(0.90 * seg_len[j - 1],
                                      terminal_out_ratio * seg_len[j]);
        } else if (j + 2 == n - 1) {
            const bool row12_right_inner_map_row1_turn =
                target_is_depot_a1 &&
                std::abs(simplified[j].x - row1_right_down_x) < 0.03 &&
                std::abs(simplified[j].y -
                         corridor_lane_y(mp_, 1, HDir::RIGHT)) < 0.03;
            if (row12_right_inner_map_row1_turn) {
                const double row12_map_tangent =
                    0.45 * std::abs(corridor_lane_y(mp_, 1, HDir::RIGHT) -
                                    corridor_lane_y(mp_, 2, HDir::LEFT));
                turn_limits[j] = std::min(
                    {0.90 * seg_len[j - 1], 0.90 * seg_len[j],
                     row12_map_tangent});
            } else if (!terminal_reverse) {
    ...
    auto fit_clear_turn = [&](size_t j, double max_tangent) {
        ...
        for (int k = 0; k < 32; ++k) {
            ...
            TurnCurve candidate =
                fit_clothoid_turn(signed_turns[j], max_curvature,
                                  std::max(sample_ds, ramp),
                                  max_tangent, sample_ds);
            if (candidate.pts.empty()) continue;
