...
bool TaskAllocator::assignDropoffLeg(
    ...
    if (!cfg_.use_a1_cycle) return false;
    (void)all;
    // The destination and A1->B path must already have been reserved before
    // B->A1 dispatch. Never invent or replace a path while the A1 transaction
    // is loading or exiting.
    if (vehicle.hasPendingDropoff() &&
    ...
    ROS_ERROR_THROTTLE(
        2.0,
        "[multi_patrol][A1 invariant] V%d reached pickup completion "
        "without its pre-reserved A1->B path",
        vehicle.id);
