...
seed="$1"
port="$2"
out_dir="/mnt/d/desktop/叉车/src/forklift_planner/logs/EXP-20260908-A1-LATE-OWNER-REGRESSION"

source /opt/ros/noetic/setup.bash
source /mnt/d/desktop/叉车/devel/setup.bash
set -u
export ROS_MASTER_URI="http://127.0.0.1:${port}"
export ROS_LOG_DIR="/tmp/forklift_a1_late_owner_seed${seed}_ros"
mkdir -p "${ROS_LOG_DIR}"
...
rosrun forklift_planner multi_vehicle_patrol_node \
  ...
  _target_only:=-1 _one_shot:=false _batch_minutes:=120 \
  ...
  _stress_quiet:=false > "${out_dir}/seed${seed}_terminal.log" 2>&1
run_rc=$?
