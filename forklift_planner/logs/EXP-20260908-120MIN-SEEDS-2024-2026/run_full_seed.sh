...
export ROS_LOG_DIR="/tmp/forklift_120min_full_seed${seed}_ros"
mkdir -p "${ROS_LOG_DIR}"

roscore -p "${port}" > "${out_dir}/seed${seed}_full_roscore.log" 2>&1 &
core_pid=$!
cleanup() {
...
rosrun forklift_planner multi_vehicle_patrol_node \
  ...
  _one_shot:=false \
  _batch_minutes:=120 \
  _coord_log_file:="${out_dir}/seed${seed}_full_coord.log" \
  _stress_watchdog_enabled:=false \
  _stress_quiet:=false \
  > "${out_dir}/seed${seed}_full_terminal.log" 2>&1
run_rc=$?
echo "SEED=${seed} FULL_RUN_RC=${run_rc}"
exit "${run_rc}"
