...
export ROS_LOG_DIR="/tmp/forklift_normal_deadlock_obb_seed2024_ros"
mkdir -p "${ROS_LOG_DIR}"

roscore -p "${port}" > "${out_dir}/seed2024_roscore.log" 2>&1 &
core_pid=$!
...
rosrun forklift_planner multi_vehicle_patrol_node \
  ...
  _coord_log_file:="${out_dir}/seed2024_coord.log" \
  _stress_watchdog_enabled:=true _stress_progress_timeout:=60.0 \
  _stress_result_file:="${out_dir}/seed2024_stress_result.txt" \
  _stress_failure_file:="${out_dir}/seed2024_stress_failure.log" \
  _stress_quiet:=false > "${out_dir}/seed2024_terminal.log" 2>&1
run_rc=$?
echo "SEED=${seed} MINUTES=${minutes} RUN_RC=${run_rc}"
exit "${run_rc}"
